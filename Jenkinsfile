...
pipeline {
	...
	stage('Push') {
	       ...
	       sh("docker -H 10.3.20.117:5555 login -u hemantkbajaj -p hunny2744")
	       sh("docker -H 10.3.20.117:5555 push ${imgtag}")	       
            }
        }
        stage('Deploy') {
            steps {
		...
		sh("kubectl apply -f ./yaml/hello-kubernetes.custom-message.yaml -n appadmin")		
            }
        }
    }
}
